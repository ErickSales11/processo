...
function listarEquipes(){
    ...
    if(equipes.length > 0){
        for(let i = 0; i < equipes.length; i++){
            listaDeEquipes.innerHTML += `
                <li>
                    <h4>${equipes[i].nome}<box-icon name="home" onClick='alert'/h4>
                    <div>
...
function addParticipante(){
    ...
    if(equipes[eId.value].participantes
        .length < equipes[eId.value].qtdMax){
            equipes[eId.value].participantes.push(pNome.value);
            localStorage.getItem('equipes', JSON.stringify(equipes))
            formAddParticipante.reset();
            listarEquipes();
        }else{
            alert('Está equipe Já está completa')
        }
